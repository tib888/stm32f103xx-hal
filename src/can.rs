//! CAN interface
//!
//! You can use the `Can` interface with these CAN instances
//!
//! # CAN1
//!
//! - TX = PA12 | PB9
...
impl Id {
    ...
    pub const fn new_standard(standard: u32) -> Id
    {
        Id {
    ...
    pub const fn new_extended(extended: u32) -> Id
    {
        Id {
...
pub struct Payload {
    ...
    //TODO: this below is not OK in case of CAN FD where the max data length is 64 byte, not only 8:
    data_low: u32,
    data_high: u32,
}

//TODO put a part of this in the HAL as Payload trait?
impl Payload {
...
//TODO put a part of this in the HAL as Frame trait?
impl Frame {
...
impl<PINS> Can<CAN, PINS> {
    ...
    pub fn configure(&mut self, config: &Configuration) {    
        ...
        self.can.can_mcr.modify( |_, w| {
            w
            .ttcm().bit(config.time_triggered_communication_mode)
            .abom().bit(config.automatic_bus_off_management)
            .awum().bit(config.automatic_wake_up_mode)
            .nart().bit(config.no_automatic_retransmission)
            .rflm().bit(config.receive_fifo_locked_mode)
            .txfp().bit(config.transmit_fifo_priority)
        });
        
        //Set the bit timing register
        self.can.can_btr.modify( |_, w| unsafe {
            w
            .silm().bit(config.silent_mode)
            .lbkm().bit(config.loopback_mode)
            .sjw().bits(config.synchronisation_jump_width)
            .ts1().bits(config.bit_segment_1 - 1)
            .ts2().bits(config.bit_segment_2 - 1)
            .brp().bits(config.time_quantum_length - 1)
        });
    ...
    pub fn activate_filter_bank(&mut self, index: FilterBankIndex, activate: bool)
    {
        ...
        if activate {
            self.can.can_fa1r.modify(|r, w| unsafe {
                w.bits(r.bits() | bit)
            });
        } else {
            self.can.can_fa1r.modify(|r, w| unsafe {
    ...
    pub fn configure_filter_bank(&mut self, index: FilterBankIndex, config: &FilterBankConfiguration)
    {
        if index >= FILTERBANKCOUNT { 
            return; // Err(Error::InvalidArgument);
        }
        ...
        match config.mode {
            FilterMode::Mask => {
                ...
                self.can.can_fm1r.modify(|r, w| unsafe {
                    w.bits(r.bits() & nbit)
                });
            },
            FilterMode::List => {
        ...
        match config.fifo_assignment {
            0 => {
                self.can.can_ffa1r.modify(|r, w| unsafe {
                    w.bits(r.bits() & nbit)
                });
            },
            1 => {
        ...
        match config.info {
            FilterInfo::Halves((ref low, ref high)) => {
                ...
                self.can.can_fs1r.modify(|r, w| unsafe {
                    w.bits(r.bits() & nbit)
                });
                ...
                self.fill_filter_registers(index,  
    ...
    fn fill_filter_registers(&self, index: FilterBankIndex, r1: u32, r2: u32) {
        let offset = (index as u32) * (((&self.can.f1r1 as *const _) as u32) - ((&self.can.f0r1 as *const _) as u32));
...
impl Tx<CAN> {
    ...
    pub fn split(self) -> (TxMailBox<CAN, TxMailBox0>, TxMailBox<CAN, TxMailBox1>, TxMailBox<CAN, TxMailBox2>) {
        (
            TxMailBox {
                _can: PhantomData,    
                _index: PhantomData, 
