...
fn main()
{
    ...
    let mut can = Can::can1(dp.CAN, (cantx, canrx), &mut afio.mapr, &mut rcc.apb1, dp.USB);

    can.configure(&config);
    nb::block!(can.to_normal()).unwrap();//just to be sure

    const ID10 : Id = Id::new_standard(10);
    const ID11 : Id = Id::new_standard(11);
    const ID12 : Id = Id::new_standard(12);
    const ID13 : Id = Id::new_standard(13);
    const ID14 : Id = Id::new_standard(14);
    const ID15 : Id = Id::new_standard(15);

    let filterbank0_config = FilterBankConfiguration {
        mode: FilterMode::List,
        info: FilterInfo::Whole(FilterData { id: ID10, mask_or_id2: ID11 }),
        fifo_assignment: 0,
        active : true,
    };
    let filterbank1_config = FilterBankConfiguration {
        mode: FilterMode::List,
        info: FilterInfo::Whole(FilterData { id: ID12, mask_or_id2: ID13 }),
        fifo_assignment: 1,
        active : true,
    };
    let filterbank2_config = FilterBankConfiguration {
        mode: FilterMode::List,
        info: FilterInfo::Whole(FilterData { id: ID14.with_rtr(), mask_or_id2: ID14 }),
        fifo_assignment: 0,
        active : true,
    };
    let filterbank3_config = FilterBankConfiguration {
        mode: FilterMode::List,
        info: FilterInfo::Whole(FilterData { id: ID15.with_rtr(), mask_or_id2: ID15 }),
        fifo_assignment: 1,
        active : true,
    };
    ...
    writeln!(hstdout, "tx: {:?} {:?} {:?}", &txresult0, &txresult1, &txresult2).unwrap();//while this printing all 3 messages are transfered
    ...
    loop {   
        if let Ok((filter_match_index, time, frame)) = rx0.read() {
            writeln!(hstdout, "rx0: {} {} {} {} {}", filter_match_index, frame.id().standard(), time, frame.data().len(), frame.data().data_as_u64()).unwrap();
        };

        if let Ok((filter_match_index, time, frame)) = rx1.read() {
            writeln!(hstdout, "rx1: {} {} {} {} {}", filter_match_index, frame.id().standard(), time, frame.data().len(), frame.data().data_as_u64()).unwrap();
        };
    }
